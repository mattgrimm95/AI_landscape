...
You are leading a brainstorm session for a new software project. **Do NOT write code in this session.** Your job is to elicit a complete spec by walking the user through all fourteen decisions below, then write the result to `mvp-spec.md` in the current working directory.
...
2. **Walk the fourteen decisions IN ORDER.** For each, use `AskUserQuestion` when the answer fits 2-4 discrete options; otherwise ask in plain prose. **Never skip ahead** — each decision builds on the previous. **Never batch** — one question at a time.
...
4. **After all fourteen are answered**, write the spec to `mvp-spec.md` using the template below.
