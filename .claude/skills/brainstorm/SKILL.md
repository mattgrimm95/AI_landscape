...
15. **Source control + publication.** Five sub-questions:
    - (a) **Project name** — kebab-case identifier used for the directory, the git repo, the package/module name (if applicable), and the Docker image tag. Pick once, used everywhere. Examples: `ai-landscape`, `my-research-tool`, `dose-tracker`. Required even if you say no to git (the directory still needs a name).
    - (b) Initialize a new git repo? (yes / no)
    - (c) Make commits during the build? (yes / no)
    - (d) Push to a remote (GitHub / GitLab / Bitbucket)? If yes: which URL (or "I'll add it later") and visibility (public / private).
    - (e) **The no-secrets guard test must pass before any push happens.** This is a non-negotiable hard gate. `/mvp-build` copies the canonical test template from `AI_landscape/tests/test_no_secrets.py` (the reference implementation in this user's other repo) into the new project's `tests/test_no_secrets.py` at scaffold time, then runs it explicitly in its Step 4 before any push. **If the template path isn't reachable on this machine, `/mvp-build` will fall back to generating an equivalent test from scratch — confirm this is acceptable now, so it doesn't surprise you mid-build.**

    Default if unsure: "yes git, commit, no push — I'll add a remote and push manually after reviewing."
...
- Project name (kebab-case): `{e.g. ai-landscape, my-research-tool}`
...
- No-secrets test template source: `AI_landscape/tests/test_no_secrets.py` (copied into new project's `tests/test_no_secrets.py` by `/mvp-build`; falls back to from-scratch generation if path unreachable)
- No-secrets gate: ACKNOWLEDGED (test must pass before any push; non-negotiable)
...
- **For decision #15:** **always require the project name** (kebab-case) — even when the user says "no git." The name becomes the directory, the package name (if applicable), and the Docker image tag if one gets built later. If they give a name in another case (snake_case, PascalCase, free text with spaces), convert it to kebab-case in the spec and confirm. If the user says "yes, push to a remote," pin down the URL and visibility NOW rather than letting them drift to the build step. If they say "I'll add the remote later," that's fine — record it so `/mvp-build` knows to init the repo + commit but skip the push. **Always reaffirm the no-secrets gate verbatim**: "the build will not push if `tests/test_no_secrets.py` finds anything sensitive — that's not an option you can override." Also confirm the test template source: `/mvp-build` will look for `AI_landscape/tests/test_no_secrets.py` to copy from; if that path isn't reachable on the user's machine, it falls back to generating an equivalent from scratch. Surface this so the user isn't surprised mid-build. If the user says "no git entirely," ask once: "Are you sure? Source control is the easiest way to revert a bad build, and the in-repo mirror lets you carry the scaffold between machines. If you change your mind later, `git init` is a one-liner." Then respect their answer.
